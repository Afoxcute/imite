...
const mintLicense = async (licenseRequest) => {
    try {
        ...
        const { txHash, blockNumber, explorerUrl } = await (0, storyService_1.mintLicenseOnFlow)(licenseRequest.tokenId, licenseRequest.royaltyPercentage, licenseRequest.duration, licenseRequest.commercialUse, licenseRequest.terms, licenseRequest.searContractAddress);
        const result = {
            success: true,
            txHash,
            blockNumber,
